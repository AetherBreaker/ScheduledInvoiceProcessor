...
class CoremarkProcessor(SupplierProcessorSFTPIntermediate):
  ...
  pickup_ftp_folder = PurePosixPath("/Sweetfire_out")
  pickup_archive_ftp_folder = None
  pre_processing_waiting_folder = PurePosixPath("/Waiting/Coremark")
  pre_processing_archive_folder = PurePosixPath("/Waiting/Coremark/Archive")
  ...
  def _create_new_merged_file(
    ...
    return FileRegisterData(
      ...
      current_week=old_file_meta.current_week,
...
if __debug__:
  for attr_name in [
    ...
    orig_attr: PurePosixPath = getattr(CoremarkProcessor, attr_name)
    new_val = PurePosixPath("/Testing") / orig_attr.relative_to("/")
    setattr(CoremarkProcessor, attr_name, new_val)
...
async def main():
  ...
  with LiveCustom(refresh_per_second=10, console=RICH_CONSOLE) as live:
    coremark = CoremarkProcessor(live.pbar)
    ...
    #   pickup_date=now,
    #   dropoff_date=now,
    #   file_pattern=coremark.assemble_filename_pattern("9893681235", now, now, True),
    #   current_week=True,
    #   _waiting_folder=coremark.pre_processing_waiting_folder,
    #   _local_copy_folder=coremark.local_pre_processing_folder,
    #   file_names={0: "9893681235_35835.txt", 1: "9893681235_35836.txt"},
    #   invoice_nums={0: "35835", 1: "35836"},
    #   pickup_success={0: True, 1: True},
    # )

    # outp = coremark._create_new_merged_file(inp)

    orders = []

    async for order in cache.schedule.walk_typed_rows():
      if order.supplier != SuppliersEnum.COREMARK:
        continue
      # if order.store != 32:
      #   continue
    ...
    for order in orders:
      await coremark.register_pickup(
    ...
    await coremark.pickup_files()

    await cache.submit_queued_writes_to_pool()

    for order in orders:
      await coremark.register_dropoff(
        storenum=order.store,
        customer_id=order.customer,
        pickup_date=now,
        dropoff_date=now,
